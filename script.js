...
function clickIgual(e) {
    ...
    if (lcdArrTotal.length > 1) {  // si lcdArrTotal = [], es porque comenzó la operacion con "/".
        multiOperation('+', '-');
...
function playOperation(i, steps, oper1, oper2) {
    switch(lcdArrTotal[i + steps]) {
        case oper1:
            total = operations[oper1](Number(lcdArrTotal[i]), Number(lcdArrTotal[i + steps]));
            lcdArrTotal.splice(i, 2);
            lcdArrTotal[i] = total;
            i = 0;
            break;
        case oper2:
            total = operations[oper2](Number(lcdArrTotal[i]), Number(lcdArrTotal[i + steps]));
            lcdArrTotal.splice(i, 2);
            lcdArrTotal[i] = total;
            i = 0;
    }
